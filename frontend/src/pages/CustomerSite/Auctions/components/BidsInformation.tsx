    ...
    return <>
        <div className={`flex flex-col border   rounded-xl   text-foreground p-5 ${new Date(props.auctionSession.endDate).getTime() < 300000 ? 'bg-red-500' : ''}`} >
            <div className='basis-4/12  flex justify-center gap-2 items-center w-full '>
                ...
                {props.auctionSession?.status === AuctionSessionStatus.PROGRESSING && new Date(props.auctionSession?.endDate) > new Date() &&
                    <>
                        <p>Time left</p>
                        <p className='font-semibold'>{props.auctionSession?.endDate ? <CountDownTime end={new Date(props.auctionSession.endDate)}></CountDownTime> : <CountDownTime end={new Date()}></CountDownTime>}</p>
                    </>}

                {props.auctionSession?.status === AuctionSessionStatus.FINISHED && new Date(props.auctionSession?.endDate) <= new Date() &&
                ...
                {props.auctionSession?.status === AuctionSessionStatus.SCHEDULED &&
                    <>
                        <p>Starts in</p>
                        <p className='font-semibold'>{props.auctionSession?.startDate ? <CountDownTime end={new Date(props.auctionSession.startDate)}></CountDownTime> : <CountDownTime end={new Date()}></CountDownTime>}</p>
                    </>}
            </div>
            <div className='basis-4/12 flex justify-center gap-2  items-center w-full '>
                <p><ArrowUp /></p>
