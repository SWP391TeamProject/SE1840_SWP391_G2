    ...
    if (!auctionSession) {
      fetchAuctionSessionById(auctionId)
        .then((res) => {
          ...
          if (res.status === AuctionSessionStatus.FINISHED) {
            setShowCofetti(true);
            setTimeout(() => {
              setShowCofetti(false);
            }, 3000);
          }
          if (res.status !== AuctionSessionStatus.SCHEDULED) {
    ...
    if (auctionSession?.status !== AuctionSessionStatus.SCHEDULED) {
      fetchBidsByAuctionId(auctionId)
        .then((res) => {
          console.log(res);
          setBids(res);
          if(res.length > 0){          setPing((prev) => [...prev, res?.sort((a, b) => - new Date(a?.createDate).getTime() + new Date(b?.createDate).getTime())[0]?.auctionItemId?.itemId]);
}         
        ...
        && bids?.filter((bid) => bid.auctionItemId.itemId === itemDTO?.itemId)[0]?.account.accountId === auth.user.accountId && (
          <div className="fixed top-0 z-10 bg-red-200/15 w-full h-full">
            ...
            <Card className="w-fit absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
              ...
              <CardContent>
                you have won this item<br />
                <Button onClick={() => navigate('/dashboard/orders')}>Click here to view your orders</Button>
              </CardContent>
            </Card>
          </div>
        )}
