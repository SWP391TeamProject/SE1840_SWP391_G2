            ...
            <div className="rounded-lg overflow-hidden shadow-lg border">
              ...
              <div className="p-6 bg-white dark:bg-gray-950">
                <div className="flex items-center gap-2 mb-2">
                  <img
                    src={blogPosts?.currentPageList[0]?.author?.avatar?.link || '/placeholder.svg'}
                    width={32}
                    height={32}
                    alt="Author Avatar"
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    {blogPosts?.currentPageList[0]?.author?.nickname} -
                  </div>
                  <div className="text-sm text-gray-500 dark:text-gray-400">Published on {blogDate}</div>
                </div>
                <h2 className="text-2xl font-bold mb-4">{blogPosts?.currentPageList[0]?.title}</h2>
                <p className="text-gray-500 dark:text-gray-400 mb-4">
                  <div
                    className="line-clamp-6"
                    dangerouslySetInnerHTML={{
                      __html: `${blogPosts?.currentPageList[0]?.content.split('&lt;img/&gt;').join(' ')}`,
                    }}
                  />
                </p>
                <Link
          ...
          {blogCategories?.map((category: BlogCategory) =>
            blogPosts?.currentPageList?.filter(
              ...
              <div
                ...
                className="grid grid-cols-1  gap-6 w-full mx-auto pt-12 pb-4  text-foreground"
              >
                      ...
                      <div className="flex gap-5 max-md:flex-col max-md:gap-0">
                        ...
                        <div className="flex flex-col ml-5 w-[67%] max-md:ml-0 max-md:w-full">
                          <div className="flex flex-col self-stretch px-5 my-auto max-md:mt-3.5 max-md:max-w-full">
                            ...
                            <p className="text-gray-500 dark:text-gray-400 line-clamp-2">
                              <div
                                dangerouslySetInnerHTML={{ __html: `${blog?.content.split('&lt;img/&gt;').join(' ')}` }}
                              />
                            </p>
                            <Link
                              onClick={() => handleViewDetailsClick(blog.postId)}
                            ...
                            <div className="flex gap-5 mt-10 w-full max-md:flex-wrap max-md:max-w-full">
                              <div className="flex flex-auto gap-0 text-sm leading-5">
                                ...
                                <div className="flex-auto">
                                  {blog?.author?.nickname} - Published on {new Date(blog?.updateDate).getDate()}/
                                  {new Date(blog?.updateDate).getMonth() < 9 ? '0' : ''}
                                  {new Date(blog?.updateDate).getMonth() + 1}/{new Date(blog?.updateDate).getFullYear()}
                                </div>
                              </div>
                            </div>
                          </div>
