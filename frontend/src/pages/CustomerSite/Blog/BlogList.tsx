    ...
    <>
      <div className='grid min-h-svh container'>
        <div className='grid-cols-12 p-1'>
            ...
            <NavigationMenu>
              <NavigationMenuList className="flex flex-wrap gap-2 mt-4">
                <NavigationMenuItem className='hover:cursor-pointer'>
                  <NavigationMenuLink
                    asChild
                    className={navigationMenuTriggerStyle() + 'border rounded-lg'}
                  >
                    <Link to={`/blogs`}>
                      All Blogs
                    </Link>
                ...
                {blogCategories?.map((category) => (
                  <NavigationMenuItem key={category.id} className='hover:cursor-pointer'>
                    <NavigationMenuLink
                      ...
                      className={navigationMenuTriggerStyle() + 'border rounded-lg'}
                    >
                      <Link to={`/blogs?categoryId=${category.blogCategoryId}`}>
                        {category.name}
                      </Link>
          ...
          <div id='blog-section' className='col-span-12 w-full'>
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-3 mx-auto'>
              {blogPosts ? blogPosts?.currentPageList.map((post, index) => (
                <BlogCard key={post.postId || index} blog={post} />
              )) :
