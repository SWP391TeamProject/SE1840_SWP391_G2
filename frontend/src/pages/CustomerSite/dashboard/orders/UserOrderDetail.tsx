...
export function UserOrderDetail() {
    ...
    const submitUpdateOrder = async () => {
        ...
        if (!ok) return
        setLoading(true);
        updateOrder(orderId, orderUpdateForm.getValues()).then((res) => {
            dispatch(setCurrentOrder(res.data));
            setLoading(false);
            ...
            <div
              className="lg:flex flex-row justify-between gap-10 items-start p-10 lg:p-20 lg:pt-10">
                <div className="basis-7/12">
                    {order.shippingStatus == ShippingStatus.PACKAGING && <div className="mb-4 flex justify-between">
                        <h2 className="text-2xl font-bold">Contact information</h2>
                        <Button onClick={() => submitUpdateOrder()} size="sm">Update</Button>
                    </div>}
                    {order.shippingStatus == ShippingStatus.PACKAGING && <div className="pb-6">
