...
export function OrderCheckout() {
  ...
  const checkout = async () => {
    ...
    if (!ok) return
    setLoading(true);
    payOrder(orderId, orderCheckoutForm.getValues()).then((res) => {
      dispatch(setCurrentOrder(res.data));
      nav("/dashboard/order/" + orderId);
    }).catch((e) => {
      setLoading(false);
    ...
    <>
      {loading ?
        ...
        <div
          ...
          <div
            className="basis-5/12 xl:basis-4/12 bg-gray-100 dark:bg-gray-800 rounded-lg p-6 mt-10 lg:mt-0">
            ...
            <div className="flex flex-col gap-5">
              ...
              <Card>
                <CardHeader>
                  <CardTitle><Wallet className="h-5 w-5 inline-block"/> Pay with
                    Biddify wallet</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="flex justify-between">
                ...
                <CardFooter>
                  <i className="text-sm">Please note that the order is
                    non-refundable.</i>
                </CardFooter>
              </Card>
              {
              ...
              <Button onClick={() => checkout()}
                      disabled={order.payment.paymentAmount - auth.user?.balance > 0}>Proceed
                to Checkout</Button>
            </div>
          </div>
        </div>
