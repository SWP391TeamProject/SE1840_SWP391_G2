  ...
  React.useEffect(() => {
    ...
    if (userCookie) {
      try {
        ...
        axios
          .get(`${SERVER_DOMAIN_URL}/api/consignments/user/${userData?.id}`, {
            headers: {
          ...
          <TableBody>
            {consignments.map((consignment) => (
              <TableRow key={consignment.consignmentId}>
                ...
                <TableCell>
                  {consignment.status == ConsignmentStatus.WAITING_STAFF ? 'Evaluating' : consignment.status}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <div className={`flex flex-row flex-wrap gap-16 ${layout === layoutType.CARD ? 'block' : 'hidden'}`}>
        {layout === layoutType.CARD &&
          ...
          consignments
            ...
            .map((consignment) => <CustomerConsigmentCard key={consignment.consignmentId} consignment={consignment} />)}
      </div>
    </div>
  );
}
