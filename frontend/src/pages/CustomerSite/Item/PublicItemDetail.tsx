...
import {
  fetchAuctionSessionHistoryOfItem
...
export function PublicItemDetail() {
  ...
  useEffect(() => {
    getItemById(itemId).then((res: AxiosResponse<Item, any>) => {
      setLoading(false);
      setItem(res.data);

      if (res.data.attachments && res.data.attachments.length > 0)
        setSelectedImageUrl(res.data.attachments[0].link);

      if (res.data.status != ItemStatus.QUEUE) {
        fetchAuctionSessionHistoryOfItem(res.data.itemId).then((res: AxiosResponse<Page<AuctionSession>, any>) => {
      ...
      {loading ?
        ...
        <div className="container w-full py-12">
          <div className="flex flex-row flex-wrap md:flex-nowrap gap-10">
              ...
              <div className="flex flex-col gap-2">
                {
                  (!item.attachments || item.attachments.length == 0) ?
                    <img src={noImagePlaceholder} alt=""
            ...
            <div
              className="basis-11/12 md:basis-5/12 order-first	md:order-none">
              ...
              <div className="flex flex-col gap-3">
                ...
                {item.status == ItemStatus.QUEUE &&
                  <Alert variant="default">
                    ...
                    <AlertDescription>
                      This item is not in any auction. Check later!
                    </AlertDescription>
                  </Alert>}
                ...
                {item.status == ItemStatus.UNSOLD &&
                  <Alert variant="default">
                    ...
                    <AlertDescription>
                      This item has not been sold successfully before.<br/>
          ...
          <div
            ...
            <div className="w-full lg:basis-4/12 xl:basis-3/12">
              <Card>
                    ...
                    <TableBody>
                      {
                        ["color", "size", "weight", "brand", "age", "material"]
                          ...
                          .map((key) => {
                            return <TableRow>
                              <TableCell
                                className="font-medium capitalize">{key}</TableCell>
                ...
                <CardFooter>
                  <i className="text-sm">Please note that item evaluations are
                    subject to error; we advise independent verification before
                    bidding.</i>
                </CardFooter>
              </Card>
            </div>
