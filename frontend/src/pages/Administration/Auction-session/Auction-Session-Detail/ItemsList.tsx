...
export default function ItemsList({ sessionItems, sessionStatus, ...props }) {
  ...
  return (
    <Card>
      <CardHeader className="px-7">
        ...
        <CardDescription>list of all lots participated in this session</CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
            <TableRow>
              ...
              {sessionStatus === 'FINISHED' && <TableHead className="hidden md:table-cell">Sold</TableHead>}
            </TableRow>
          </TableHeader>
          <TableBody>
            {sessionItems?.map((item): any => (
              <TableRow className="bg-accent" key={item?.id.itemId}>
                ...
                {sessionStatus === 'FINISHED' && (
                  <TableCell className="hidden md:table-cell">{item?.sold ? 'Yes' : 'No'}</TableCell>
                )}
              </TableRow>
            ))}
          </TableBody>
