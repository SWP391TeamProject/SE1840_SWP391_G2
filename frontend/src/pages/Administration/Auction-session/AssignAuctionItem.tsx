...
import {
  ConfirmationButton
} from '@/components/confirmation/confirmation-button';
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-4 md:p-6">
        ...
        <div className="border rounded-lg shadow-sm">
          <div
            className="bg-gray-100 dark:bg-gray-800 px-4 py-3 border-b rounded-t-lg">
                ...
                {availableItems &&
                  availableItems.map((item: any) => (
                    <TableRow key={item.id}>
                      <TableCell
                        className="font-medium">{item.itemId}</TableCell>
                      <TableCell className="font-medium">
                        <Link to={`/admin/items/${item.itemId}`}
                              target="_blank">
                          {item.name}
                        </Link>
                      </TableCell>
                      <TableCell
                        className="font-medium">{currency.format(item.reservePrice)}</TableCell>
                      <TableCell>
