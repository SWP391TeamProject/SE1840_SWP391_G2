  ...
  const Action = () => {
    switch (consignment.status) {
      case ConsignmentStatus.WAITING_STAFF: {
        return (<ConsignmentStaffTakeButton consignment={consignment}
                                            setConsignment={setConsignment}/>)
      }
      case ConsignmentStatus.IN_INITIAL_EVALUATION: {
        return (<ConsignmentInitialEvaluationForm consignment={consignment}
                                                  setConsignment={setConsignment}/>)
      }
      case ConsignmentStatus.SENDING: {
        return (<ConsignmentStaffItemReceiveButton consignment={consignment}
                                                   setConsignment={setConsignment}/>)
      }
      case ConsignmentStatus.IN_FINAL_EVALUATION: {
        ...
        if (details.length > 0) {
          ...
          if (latest.type === ConsignmentDetailType.FINAL_EVALUATION) {
            return (<ManagerFinalEvaluationForm consignment={consignment}
                                                setConsignment={setConsignment}/>)
          }
        }
        return (<ConsignmentFinalEvaluationForm consignment={consignment}
                                                setConsignment={setConsignment}/>)
      }
