...
export default function SendEvaluationForm({ consignmentParent }: { consignmentParent: Consignment }) {
  ...
  useEffect(() => {
    // console.log(param);
    fetchConsignmentByConsignmentId(consignmentParent.consignmentId)
      .then((res) => {
        ...
        <AlertDialogHeader>
          <AlertDialogTitle>
            <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">
              {consignmentParent && consignmentParent?.status === 'IN_INITIAL_EVALUATION' ? 'Initial' : 'Final'}{' '}
              Evaluation Form
            </h1>
            <div>
              <p className=" text-gray-500 dark:text-gray-400">Fill out the form below to submit your evaluation</p>
            </div>
          </AlertDialogTitle>
        </AlertDialogHeader>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {consignmentParent?.status === 'IN_INITIAL_EVALUATION' ? 'Initial' : 'Final'} Evaluation Price:
                      </FormLabel>
                      <FormControl>
