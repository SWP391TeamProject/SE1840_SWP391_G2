...
export default function PaymentsList() {
  ...
  const handleFilterClick = (types: PaymentType[], filter: string) => {
    ...
    if (filter == 'All') {
      setPaymentPromise(
        getPayments({ page: Number.parseInt(pageNumber), size: Number.parseInt(pageSize), sort: sort, type: '' })
      );
      // fetchPayments(0);
      setTypeFilter('');
      ...
      setPaymentPromise(
        getPayments({ page: Number.parseInt(pageNumber), size: Number.parseInt(pageSize), sort: sort, type: filter })
      );
      // console.log(types);
