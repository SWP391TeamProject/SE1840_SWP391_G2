...
import WithdrawFormHandle from '@/pages/Administration/payment/WithdrawFormHandle.tsx';
    ...
    cell: ({ row }) => {
      return (
        <AccountTooltip account={row.original.account}>
          <Link to={`/admin/account/${row.original.account.accountId}`}>{row.original.account.nickname}</Link>
        </AccountTooltip>
      );
    },
    enableSorting: true,
    ...
    cell: ({ row }) => {
      switch (row.original.type) {
        ...
        case PaymentType.AUCTION_DEPOSIT: {
          return (
            <Button size="sm" asChild>
              <Link to={`/admin/auction-sessions/${row.original.depositAuctionId}`} target="_blank">
                View auction
              </Link>
            </Button>
          );
        ...
        case PaymentType.AUCTION_ORDER: {
          return (
            <Button size="sm" asChild>
              <Link to={`/admin/orders/${row.original.id}`} target="_blank">
                View order
              </Link>
            </Button>
          );
